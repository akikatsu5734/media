...
記事内容を考えるための調査ソースとしては、公的情報に加えて、民間記事・業界団体の記事・不動産会社の記事・専門家コラムなどを参考にしてよい。

理由：
- 記事の網羅性を高めるため
- 実務上の注意点やよくあるトラブルを拾うため
- 読者にとって役立つ観点を補うため

ただし、民間情報を本文に反映する場合は、必ず慎重な表現にする。

**使ってよい表現（民間情報を本文に反映する場合）：**
- 「一般的には〜とされています」
- 「〜と言われています」
- 「〜という見解もあります」
- 「〜とも解釈できます」
...
公的情報と矛盾する民間情報は採用しない。
法律・制度・税務・契約に関する最終判断が必要な内容は、自治体・法務局・税務署・専門家への確認を促す。

### 記事内の「公的情報・参考ページ一覧」に掲載してよいリンク

記事内の「公的情報・参考ページ一覧」に掲載するリンクは、原則として一次情報・公的情報のみとする。
...
- 法務局・裁判所
- e-Gov法令検索
- その他、制度・法律・行政手続きの一次情報にあたる公的機関

**原則掲載しない：**
- 民間ブログ・SEO記事・まとめサイト・個人サイト・出典不明ページ
- 不動産会社の営業記事
- 専門家コラム

**「本文生成の参考」と「記事内リンク集への掲載」は別物として扱う。**
民間情報を本文の調査参考として使うこと自体は禁止しない。ただし、リンク集への掲載は原則不可。
...
- e-Gov法令検索の `lawid` は絶対に推測しない
- `document?lawid=...` の深いURLを使う場合は、法令名とLawIdが一致していると確認できる場合のみ使用する
- 法令名とLawIdの一致が確認できない場合は、深いURLを使わない
  - 代替：e-Gov法令検索トップ（`https://laws.e-gov.go.jp/`）または検索ページへの導線にする
- e-Govリンクの最終URLが `/notfound` になる場合は使用禁止
- リンクテキストが「e-Gov法令検索｜借地借家法」なのに、リンク先が借地借家法ではない場合は使用禁止
- 法令名を本文で扱う場合も、断定しすぎず、必要に応じて専門家確認を促す
